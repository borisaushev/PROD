...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
  ...
  CREATE TABLE users (
    id serial PRIMARY KEY,
    login VARCHAR(30) UNIQUE CHECK(char_length(login) >= 1),
    email VARCHAR(50) UNIQUE CHECK(char_length(email) >= 1),
    password VARCHAR(1000),
    country_code VARCHAR(2),
    is_public BOOLEAN,
    user_phone VARCHAR(20),
    user_image VARCHAR(200) CHECK(char_length(user_image) >= 1)
  );
